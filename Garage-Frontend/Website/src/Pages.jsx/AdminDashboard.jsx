...
try{
   const response= await axios.get('https://localhost:5000/api/intake');
  ...
  setLoading(false);
    ...
    const handleStatusUpdate = async(id,newStatus)=>{
      try{
        const response = await axios.put(`http://localhost:5000/api/intake/${id}, {status: newStatus}`);
if(response.data.success){
...
<span className='w-2 h-2 rounded-full text-teal-600 animate-ping'></span>
...
<button className='py-2 bg-emerald-600/20 border border-emerald-500/30 text-amber-400 text-xs font-mono uppercase tracking-wider hover:bg-amber-500/30 transition-all disabled:opacity-30 disabled:pointers-events-none ' onClick={()=>
  handleStatusUpdate(item._id,'in-progress')
} disabled={item.status==='in-progress'}>
  In Progress
</button>
<button onClick={()=> handleStatusUpdate(item._id,'completed')} disabled={item.status==='completed'} className='py-2 bg-emerald-600/20 border border-emerald-500/30 text-amber-400 text-xs font-mono uppercase tracking-wider hover:bg-amber-500/30 transition-all disabled:opacity-30 disabled:pointers-events-none '>
Complete
</button>
